...
arglist.append(('Noise Ratio', args.noise_ratio))
...
model = model.RNNModel(args.model, ntokens, args.emsize, args.nhid, args.nlayers, args.dropout, args.tied, nce=args.loss).to(device)

def build_criterion():
    noise = build_unigram_noise(
        torch.FloatTensor(corpus.dictionary.unigram)
    )
